...
do
	ipython anomaly_detection.py -- --data ecg --filename "$idx" --save_fig True 
done

for idx in "${respiration_filenames[@]}"
do
	ipython anomaly_detection.py -- --data respiration --filename "$idx" --save_fig True
done

for idx in "${space_shuttle_filenames[@]}"
do
	ipython anomaly_detection.py -- --data space_shuttle --filename "$idx" --save_fig True
done
for idx in "${gesture_filenames[@]}"
do
	ipython anomaly_detection.py -- --data gesture --filename "$idx" --save_fig True
done

for idx in "${power_demand_filenames[@]}"
do
	ipython anomaly_detection.py -- --data power_demand --filename "$idx" --save_fig True 
done

for idx in "${nyc_taxi_filenames[@]}"
do
	ipython anomaly_detection.py -- --data nyc_taxi --filename "$idx" --save_fig True
done
